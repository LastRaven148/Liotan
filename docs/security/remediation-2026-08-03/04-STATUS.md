...
| --- | --- | --- | --- | --- | --- |
| SEC-2026-08-001 | FIXED | `10d3d76`, `6b160ac` | 5 independent media route tests; 5 low-level stream tests; full unit/security/media gates | Live R2 was intentionally not accessed | Exact status/range/length, client abort, commit barrier, access, lifecycle, device and quota paths covered |
...
| SEC-2026-08-006 | FIXED | `7f6edee` | 3 scanner GETs; 6 security-page unit; session and live-socket revocation | The raw capability necessarily appears in the email URL, browser address/history, one form action, and potentially external proxy/CDN/mail-scanner logs; only application request-context logging is proven redacted | GET is read-only; POST is explicit; operation-scoped lock ownership enforced |
