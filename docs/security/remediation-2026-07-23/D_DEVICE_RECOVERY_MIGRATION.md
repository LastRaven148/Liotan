...
`DEVICE_AUTH_V2_ENFORCED_AT` and `DEVICE_AUTH_V1_REQUESTS_DISABLED_AT` are
explicit production rollout decisions; there is no built-in calendar default.
Production startup rejects missing, malformed or reversed values. The second
timestamp retires normal v1 requests, registration, approval and renewal
independently from the enrollment cutoff. The only remaining v1 operation is an
atomic v1-to-v2 migration with old-key and fresh local v2-key proofs.
`auditDeviceAuthV1Inventory.js` is permanently count-only.

The bounded migration route also supports a user who has logged in again. The
authenticated account, active v1 record, old request-key proof, fresh v2-key
proof, root-signed manifest and signed directory transition are all required;
the compare-and-set uses the record's prior session hash and atomically moves
the device to the current session binding. The old session can no longer
authenticate that device.

After forward migrations begin, every automatic restoration of the previous
backend requires compatible protocol-generation metadata, including migration
failure and candidate failure before frontend cutover. After client-facing
cutover the same rule applies. An incompatible or unavailable rollback stops
the backend fail-closed for a forward fix.
