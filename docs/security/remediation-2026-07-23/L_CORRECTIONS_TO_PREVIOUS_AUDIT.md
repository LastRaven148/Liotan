...
| All historical CodeQL wording described current results. | Alert counts and states were a dated snapshot. The matrix distinguishes concrete fixed sinks from later verified branch checks/ruleset state; historical `main` alerts remain a separate triage backlog. |
