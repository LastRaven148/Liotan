import { CipherSuite } from "@wireapp/core-crypto/browser";
import { textEncoder } from "../encoding";

export const MLS_CIPHER_SUITE = CipherSuite.Mls128Dhkemx25519Aes128gcmSha256Ed25519;
export const CORE_CRYPTO_WASM_URL = new URL(
  "../../../node_modules/@wireapp/core-crypto/dist/browser/autogenerated/wasm-bindgen/index_bg.wasm",
  import.meta.url
).href;
export const KEY_PACKAGE_LIFETIME_SECONDS = 30 * 24 * 60 * 60;
export const MEDIA_MAGIC = textEncoder.encode("LIOTANMLS1\0\0");
export const MEDIA_CHUNK_SIZE = 1024 * 1024;
export const MEDIA_CHUNK_SIZES = Object.freeze([256 * 1024, 512 * 1024, MEDIA_CHUNK_SIZE]);
const configuredSelfUpdateHours = Number(import.meta.env.VITE_MLS_SELF_UPDATE_HOURS || 72);
export const SELF_UPDATE_INTERVAL_MS = Math.min(168, Math.max(24, configuredSelfUpdateHours)) * 60 * 60 * 1000;
export const BACKGROUND_MAINTENANCE_INTERVAL_MS = 45 * 1000;
export const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
export const MESSAGE_REFERENCE_RE = /^[A-Za-z0-9_-]{1,80}$/;
export const SAFE_MEDIA_MIMES = Object.freeze({
  photo: new Set(["image/jpeg", "image/png", "image/webp", "image/gif"]),
  video: new Set(["video/mp4", "video/webm", "video/quicktime"]),
  audio: new Set(["audio/mpeg", "audio/mp3", "audio/mp4", "audio/aac", "audio/ogg", "audio/wav", "audio/webm"])
});
