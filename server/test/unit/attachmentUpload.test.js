...
const uploadNamePattern = /^\d{13}-[0-9a-f]{32}\.upload$/;

function listTestUploads() {
  return new Set(
    fs.readdirSync(uploadTmpDir)
      .filter(name => uploadNamePattern.test(name))
  );
}

function cleanupNewTestUploads(baseline) {
  for (const name of listTestUploads()) {
    if (baseline.has(name)) continue;
    const candidate = path.resolve(uploadTmpDir, name);
    if (path.dirname(candidate) !== uploadTmpDir || !uploadNamePattern.test(path.basename(candidate))) continue;
